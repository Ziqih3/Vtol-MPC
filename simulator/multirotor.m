classdef multirotor < handle
    ...
    methods
        ...
        function SetRotorAngles(obj, RotorInwardAngles, RotorSidewardAngles, RotorDihedralAngles)
            ...
            % Inputs can be scalar or an array of the same length as 
        ...
        function wrench = CalcGeneratedWrench(obj, plantinput)
            ...
            F_x = [obj.GetGravityMoment(RBI) - cross(obj.State.Omega,... 
                   obj.I * obj.State.Omega);
...
%             wrench = F_x + F;
        ...
        function new_state = CalcNextState(obj, wrench, Thrust_Body,tf_sensor_wrench, ...
         ...
         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            new_state.LastEulerRate = obj.State.EulerRate;
            new_state.LastThrust = Thrust_Body;
            % Calculate the tilt
            ...
            for i = 1 : obj.NumOfRotors
                ...
                new_state.Throttle(i) = obj.Rotors{i}.GetCurrentThrottle(rs);
        ...
        function T_Body = CalcBodyThrust(obj,plantinput)
            RBI = obj.GetRotationMatrix();
            J_x = [obj.NE_M; RBI' * obj.NE_L];
